...
### 为什么 Day33 前接入 Spec Kit

可以这样讲：

> 第一阶段我已经有了 30 天 roadmap、开发日志和测试门禁。进入第二阶段后，功能会开始跨更多模块，例如真实 retry 联调、embedding provider、真实 LLM prompt、E2E 和 LLMOps。为了避免后续只是堆功能，我在 Day33 前接入了 GitHub Spec Kit，把流程升级成 Spec-Driven Development。

重点不是“用了一个新工具”，而是说明流程变成：
...
- Spec Kit 负责把需求、接口契约、验收标准和非目标先写清楚。
- TDD 负责把规格落成会失败、会通过、能回归的测试。
- verification-loop 负责防止功能只在局部能跑。
- 开发日志和面试文档负责沉淀“为什么这样设计”和“遇到问题怎么处理”。

如果面试官问“为什么不是一开始就上 Spec Kit”，可以回答：

> 一开始项目更需要先打通基础架构和主链路。等 Day1-Day32 已经有后端、前端、任务、报告、证据链、retry 和 CI 后，再接入 Spec Kit 更合理，因为这时候项目已经有足够多的跨模块契约，SDD 能解决真实的协作和一致性问题，而不是空转写文档。
...
> 前两个阶段证明了工程化能力：异步任务、状态持久化、RAG、报告、证据链、retry、E2E、LLMOps 和 release gate。现在我补的是实际应用闭环：CSV/JSON 评论导入、低风险 JSON-LD 页面适配、评论分析质量评估、evidence-bound LLM 报告和前端证据链展示。这样项目不再依赖“必须爬到数据”，而是能处理运营人员真实可获得的评论文件。

如果面试官问“为什么先做 CSV/JSON，而不是继续爬虫”，回答：

> 因为电商运营场景里，评论数据经常可以从店铺后台、第三方工具或人工整理文件导出。相比一开始挑战强反爬，CSV/JSON 导入更稳定、更合法，也更能证明分析链路价值。爬虫可以作为补充输入，但不能成为项目唯一数据来源。

如果面试官问“怎么证明分析有用”，回答：

> 我用 query 和 expected evidence 建 RAG 评估集，比如质量差、退货、物流慢、客服差。系统必须能召回对应评论证据，再让 LLM 在 evidence refs 约束下生成报告。最终前端能从结论看到引用证据和原始评论，这样报告不是模型自由发挥，而是有证据链。
